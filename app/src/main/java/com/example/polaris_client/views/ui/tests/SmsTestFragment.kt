...
class SmsTestFragment : Fragment(), NetworkTestService.SmsDeliveryListener {
    ...
    override fun onCreateView(
        ...
        sharedPreferences = requireContext().getSharedPreferences("sms_test_settings", Context.MODE_PRIVATE)
        ...
        scheduleSwitch.setOnCheckedChangeListener { _, isChecked ->
            if (isChecked) {
                if (phoneInput.text.toString().isEmpty() || messageInput.text.toString().isEmpty()) {
                    Toast.makeText(context, "Please enter phone number and message first", Toast.LENGTH_SHORT).show()
    ...
    override fun onRequestPermissionsResult(
        ...
        if (requestCode == SMS_PERMISSION_REQUEST_CODE) {
            if (grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                Toast.makeText(context, "SMS permission denied. Cannot run test.", Toast.LENGTH_SHORT).show()
    ...
    private fun runTest(phoneNumber: String, message: String) {
        ...
        // Run SMS test with timeout
        networkTestService.runSmsTest(phoneNumber, message, this)
        
        // Set timeout for SMS delivery
        Handler(Looper.getMainLooper()).postDelayed({
            if (startButton.isEnabled.not()) {  // If test is still running
                onSmsDeliveryFailed("Delivery timeout after ${SMS_DELIVERY_TIMEOUT/1000} seconds")
            }
        }, SMS_DELIVERY_TIMEOUT)
    }
    
    override fun onSmsDelivered(deliveryTimeMs: Long) {
        activity?.runOnUiThread {
            progressBar.visibility = View.GONE
            startButton.isEnabled = true
            resultText.text = "SMS delivery time: ${deliveryTimeMs} ms (${formatReadableTime(deliveryTimeMs)})"
            loadPreviousResults()
        }
    }
    ...
    class SmsAlarmReceiver : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            if (intent.action == "com.example.polaris_client.SMS_TEST_ALARM") {
                ...
                networkTestService.runSmsTest(phoneNumber, testMessage, object : NetworkTestService.SmsDeliveryListener {
                    override fun onSmsDelivered(deliveryTimeMs: Long) {
                        // Log the test result to database when running in background
                        dbHelper.insertNetworkTestData(
                            "SMS",
                            deliveryTimeMs.toString(),
                            "To: $phoneNumber, Msg: $testMessage",
                            0.0, // latitude
                            0.0  // longitude
                        )
                    }
